...
const RULES = [
  ...
  ["타일", "채움 있음. 유일한 테두리는 종목 성격 헤어라인. 채움이 차트 표면과 같은 값이라 스파크라인이 가장자리까지 흐른다."],
  ["타일 패딩", "14px 좌우 / 12px 상단 / 0 하단. 하단이 0인 건 스파크라인이 전폭으로 붙기 때문."],
  ["그리드 간격", "12px. 열은 3 / 2 / 1 (≥1024 / ≥640 / 그 이하)."],
  ["컨트롤", "상단 밴드가 아니라 하단 플로팅 리모컨. 눈이 이미 데이터에 가 있으므로 손도 거기 닿아야 한다. 콘텐츠 하단에 112px 여백을 둬 마지막 타일이 가리지 않게 한다."],
...
export function DensitySection() {
  return (
    <SpecSection
      id="density"
      title="밀도와 형태"
      intro="그리드 뷰에서 화면에 그어지는 선은 스파크라인과 타일의 성격 헤어라인뿐이다. 열 구분선도, 그룹 구분선도 없다."
    >
